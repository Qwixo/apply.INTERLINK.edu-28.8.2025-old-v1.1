    ...
    <section className="relative w-full min-h-screen flex items-center justify-center overflow-hidden px-4">
      ...
      <Floating sensitivity={-0.025} className="absolute inset-0 pointer-events-none z-10">
        <FloatingElement depth={1.2} className="top-[4%] left-[10%] block md:hidden">
          <motion.img src={images[1]} alt="img-1" className="w-40 h-28 object-cover rounded-xl shadow-2xl rotate-[4deg]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} />
        </FloatingElement>
        <FloatingElement depth={1.2} className="top-[4%] right-[10%] block md:hidden">
          <motion.img src={images[3]} alt="img-3" className="w-40 h-28 object-cover rounded-xl shadow-2xl -rotate-[3deg]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} />
        </FloatingElement>
...
>
  <motion.span layout className="block">Biến giấc mơ Mỹ của bạn</motion.span>
  <LayoutGroup>
    <motion.span layout className="inline-flex flex-wrap whitespace-pre items-center justify-center">
      <TextRotate
        texts={['thành tiết kiệm', 'thành khả thi', 'thành thực tế', 'thành 🔥🔥🔥']}
      ...
      />
        ...
        >
          Học tiếng Anh, khám phá văn hóa Mỹ, kết bạn mới và chuẩn bị cho tương lai của bạn tại INTERLINK.
...
>
  Đăng Ký Ngay và Bắt Đầu
      ...
      >
        <svg className="w-6 h-6 text-[#064088] animate-bounce" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
