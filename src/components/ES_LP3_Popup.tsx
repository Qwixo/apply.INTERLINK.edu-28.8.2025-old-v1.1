...
type Props = {
  open: boolean
  onClose: () => void
  guideUrl: string                 // p. ej. "/es/guide"
  redirectUrl?: string             // p. ej. enlace de solicitud Orbund
  tag?: string                     // etiqueta de Mailchimp; identifica el popup LP3 ES
}

const MC_U = '607289d66021f9dbd8e30d04c'
const MC_ID = 'f16b13f8cc'
// ⚡️ mismo label de conversión que el botón original
const ADS_SEND_TO = 'AW-1043003990/KpaYCO2GlLQaENb0q_ED'

export default function ES_LP3_Popup({
  open,
  onClose,
  guideUrl,
  redirectUrl,
          ...
          >
            ...
            <div className="text-center mb-4">
              <div className="text-xs md:text-sm uppercase tracking-wide text-[#064088] font-semibold">
                ¡HOY TE LLEVAS LA GUÍA GRATIS DE REGALO!
              </div>
              <h2 className="text-2xl md:text-3xl font-bold text-[#064088] mt-2">
                INGRESA TUS DATOS PARA CONTINUAR TU SOLICITUD
              </h2>
              <div className="text-[#064088]/90 mt-1">
                Si aún no la recibiste, te enviaremos la guía por correo electrónico.
              </div>
            </div>
            ...
            <form
              onSubmit={async (e) => {
                ...
                data.append('UTM_CAMP', utm.utm_campaign || 'lp3-es-colombia-or-mexico')
                data.append('UTM_CONT', utm.utm_content || '')
                data.append('UTM_TERM', utm.utm_term || '')
                data.append('MMERGE17', tag)
                data.append('COUNTRY', 'ColombiaOrMexico');
                ...
                try {
                  ...
                  console.warn('Mailchimp post falló (posible sin conexión). Continuamos…', err)
                }

                // ✅ Igual que el botón: abrir Orbund en una nueva pestaña
                const openInNewTab = () => {
                  const dest =
                    redirectUrl ||
                  ...
                  // Opcional: también mostrar la guía en la pestaña actual
                ...
                if (typeof window !== 'undefined' && (window as any).gtag && navigator.onLine) {
                  ...
                  // fallback solo si el callback no se dispara
                  setTimeout(cb, 1000)
                } else {
                  console.warn('❌ gtag no definido o sin conexión — abriendo la solicitud directamente')
                  openInNewTab()
                }
              }}
              className="space-y-3"
            >
              ...
              >
                <span>Continuar y obtener la guía</span>
                <ArrowRight size={16} />
              </button>
            </form>
